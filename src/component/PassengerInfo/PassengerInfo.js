...
const PassengerInfo = ({ onConfirm, flightData = {} }) => {
  ...
  useEffect(() => {
    fetch("http://localhost:8080/api/passengers/passagers")
      .then((response) => {
        if (!response.ok) {
  ...
  const findPassengerIdByName = (firstName, lastName) => {
    ...
    const foundPassenger = allPassengers.find(
      (p) =>
        p.password.toLowerCase() === firstName.toLowerCase() &&
        p.first_name.toLowerCase() === lastName.toLowerCase()
    );
  ...
  const handleFinalConfirmation = () => {
    let mainPassengerId = findPassengerIdByName(
      passengers[0].first_name,
      passengers[0].last_name
    );
    console.log("le passager name ", passengers[0].first_name);
    console.log("le passager principal", mainPassengerId);
    if (!mainPassengerId) {
  ...
  return (
    <div className="bg-white p-6 shadow-lg rounded-lg max-w-4xl mx-auto">
      ...
      {passengers.map((passenger, index) => (
        <div
          key={index}
          className="mb-4 p-4 border rounded-lg grid grid-cols-1 md:grid-cols-2 gap-4"
        >
          <input
            type="text"
            placeholder="Prénom"
          ...
          <input
            ...
            onChange={(e) =>
              handleInputChange(index, "num_pass", e.target.value)
            }
            className="border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-blue-500"
          />
          <input
          ...
          <input
            ...
            onChange={(e) =>
              handleInputChange(index, "date_naissance", e.target.value)
            }
            className="border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-blue-500"
          />
        </div>
      ...
      <div className="flex justify-between mt-6">
        <button
          ...
          className="btn btn-primary bg-blue-500 text-white py-2 px-4 rounded-lg hover:bg-blue-700"
        >
          Ajouter un passager
        </button>
        <button
          onClick={handleFinalConfirmation}
          className="btn btn-success bg-green-500 text-white py-2 px-4 rounded-lg hover:bg-green-700"
        >
