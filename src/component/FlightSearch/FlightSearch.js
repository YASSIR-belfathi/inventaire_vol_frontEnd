...
const FlightSearch = ({ onFlightSelect }) => {
  ...
  return (
    <div className="bg-white p-6 shadow-lg rounded-lg">
      <h2 className="text-2xl font-bold text-blue-700 mb-6">Rechercher un vol</h2>
      ...
      <div className="mt-4">
        {flights.map((flight) => (
          ...
          >
            <p>De: {flight.destinationDepart} - À: {flight.destinationArrivee}</p>
            <p>Date: {flight.dateDepart} - Prix: {flight.prix}€</p>
          </div>
        ))}
      </div>
    </div>
